...
const SECRET_PATTERNS: Array<{
  ...
  {
    // Ark endpoint or resource ID tokens: ep- followed by timestamp/id (e.g. ep-20240830123456-abcde, ep-m-20240830123456, ep-abcdefghijkl)
    // Anchored so URLs like https://example.com/docs/ep-getting-started-guide do not match.
  ...
  {
    // Authorization Bearer tokens in header context (e.g. Authorization: Bearer <token>, authorization: bearer <token>)
    regex: /\b(?:authorization|auth)\s*[:=]\s*['"]?bearer\s+['"]?[a-zA-Z0-9_\-\.]{20,}['"]?/gi,
    kind: "token-like",
...
export function scanForSecrets(raw: string): RedactionFinding[] {
  ...
  // Floor of 8 characters protects shorter real keys (e.g. 8 and 10 chars)
  // while preventing short generic dev values like "devkey" (6 chars) from false-positive substring matches.
