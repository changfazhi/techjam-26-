...
export function PipelinePanel({ agentId, onClose }: PipelinePanelProps) {
  ...
  return <section className={"pipeline-panel" + (live ? "" : " is-fixture")} aria-labelledby="pipeline-title">
    <header className="pipeline-panel-head"><div><span className="eyebrow">Handoff Gate</span><h2 id="pipeline-title">Pipeline</h2><p>{session.title}</p></div><div className="pipeline-actions"><button className="button button-danger" type="button" onClick={stop} disabled={!live || session.state !== "running" || stopping}>{stopping ? "Stopping..." : "Stop"}</button><button className="button button-ghost" type="button" onClick={onClose}>Close</button></div></header>
